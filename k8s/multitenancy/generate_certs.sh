...
CONFIG=${CONFIG?CONFIG is required (e.g. whitley, flex, opus etc.)}
...
#openssl req -new -key "${tmpdir}/server-key.pem" -subj "/CN=${fullServiceDomain}" -out "${tmpdir}/server.csr" -config "${tmpdir}/csr.conf" -days 3650
...
#'/CN=system:node:node-name-here;/O=system:nodes'
...
cp webhook.yaml out/$CONFIG
cp webhook-registration.yaml out/$CONFIG
sed -i -e 's/caBundle:.*$/caBundle: '${caBundle}'/' out/${CONFIG}/webhook-registration.yaml
